...
// Génère un jeu de données de démonstration cohérent pour présenter l'outil.
// Toutes les données créées sont taguées « DÉMO — » (clients, offres) et donc
// re-générables / supprimables sans toucher aux vraies données importées.
// action = 'generer' (défaut) : (re)crée le jeu de démo.
// action = 'supprimer' : supprime uniquement les données de démo.
...
const NOMS = ['EARL des Grands Champs', 'GAEC du Moulin', 'SCEA de la Prairie', 'CUMA de la Vallée', 'EARL Bellevue', 'Ferme des Trois Chênes', 'GAEC Terres Neuves', 'ETA Servibois', 'EARL du Verger', 'SCEA Les Peupliers', 'GAEC de la Fontaine', 'EARL Champ Fleuri', 'Exploitation Roussel', 'CUMA du Bocage', 'GAEC des Ormes', 'EARL La Genête', 'Ferme du Coteau', 'SCEA du Grand Pré', 'ETA Duval', 'EARL Les Sablons', 'GAEC Saint-Martin', 'EARL du Pont', 'Domaine de la Rivière', 'GAEC Les Acacias'];
const COMMUNES = ['Melle', 'Niort', 'Issoudun', 'Naintré', 'Chasseneuil', 'Sauzé', 'Vasles', 'Châtillon', 'Saint-Maur', 'Noyers-sur-Cher'];
...
async function supprimerDemo(sr) {
  const clients = (await listAll('client', sr)).filter((c) => (c.raison_sociale || '').startsWith(PREFIX));
  const clientIds = new Set(clients.map((c) => c.id));
  const [rdvs, ventes, materiels, offres] = await Promise.all([
    listAll('rdv', sr), listAll('vente', sr), listAll('materiel', sr), listAll('offre_magasin', sr)
  ]);
  await del('rdv', rdvs.filter((r) => clientIds.has(r.client_id)).map((r) => r.id), sr);
  await del('vente', ventes.filter((v) => clientIds.has(v.client_id)).map((v) => v.id), sr);
  await del('materiel', materiels.filter((m) => clientIds.has(m.client_id)).map((m) => m.id), sr);
  await del('offre_magasin', offres.filter((o) => (o.titre || '').startsWith(PREFIX)).map((o) => o.id), sr);
  await del('client', [...clientIds], sr);
...
export default async function(req) {
  try {
    ...
    if (body.action === 'supprimer') {
      const n = await supprimerDemo(sr);
      return Response.json({ ok: true, supprimes: n });
    }
    ...
    const managerByUser = {}; // user_id commercial -> user_id responsable
    const respByName = {};
    users.filter((u) => u.app_role === 'responsable').forEach((u) => { if (u.full_name) respByName[u.full_name.toLowerCase()] = u.id; });
    structure.forEach((s) => {
      if (s.user_id && s.manager && respByName[s.manager.toLowerCase()]) managerByUser[s.user_id] = respByName[s.manager.toLowerCase()];
    });

    // 2. Badges (catalogue) si absent
    ...
    if (badges.length === 0) {
      const cat = [
        ...
        { nom: 'Série active', description: '3 jours consécutifs actifs', icone: '🔥', condition_type: 'serie_active', condition_seuil: 3 }
      ];
      badges = await sr.entities.badge.bulkCreate(cat);
    }

    // 3. Paramètres opération (dates + objectifs) si non renseignés
    ...
    const clientsPayload = NOMS.map((nom, i) => {
      ...
      return {
        raison_sociale: PREFIX + nom,
        siren: 'DEMO' + (100000000 + i),
        type_structure: ts,
        adresse_complete: `${rndInt(1, 60)} route de ${rnd(COMMUNES)}, ${rnd(COMMUNES)}`,
        tel_mobile: '06' + rndInt(10000000, 99999999),
        sources_donnees: ['MISTRA'],
    ...
    // 5. Matériel (1 à 3 par client)
    const materiels = [];
    clients.forEach((c) => {
      for (let k = 0; k < rndInt(1, 3); k++) {
        materiels.push({
          client_id: c.id, marque: rnd(MARQUES), modele: 'T' + rndInt(4, 8) + '.' + rndInt(100, 320),
          occasion_neuf: Math.random() > 0.5 ? 'Neuf' : 'Occasion',
          statut_possession: 'Propriétaire', propriete: 'Propriétaire',
          age_estime: rndInt(1, 15), famille_materiel: 'Matériel agricole',
          commerciaux_assignes: c.commerciaux_assignes || []
        });
      }
    });
    if (materiels.length) await sr.entities.materiel.bulkCreate(materiels);

    // 6. RDV : réalisés (ce mois, dont aujourd'hui) + planifiés à venir
    const rdvs = [];
    clients.forEach((c, i) => {
    ...
    // Quelques RDV réalisés aujourd'hui pour le compteur du jour
    for (let j = 0; j < 4; j++) {
      const c = clients[j];
    ...
    if (rdvs.length) await sr.entities.rdv.bulkCreate(rdvs);

    // 7. Ventes : tous types machine, mix validées / à valider, quelques reprises
    const ventes = [];
    for (let i = 0; i < 22; i++) {
      ...
      ventes.push({
        ...
        date_vente: dateOnly(daysFromNow(-rndInt(0, 20))),
        type_machine: MACHINES[i % MACHINES.length],
